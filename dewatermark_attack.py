...
def main():
    ...
    test_instances: List[DataInstance] = dataset_processor.load_jsonl(DATASET_DIR,
                                                                      split='test')
    ...
    with torch.no_grad():
        ...
        for bid, batch in enumerate(prog):
            ...
            watermark = torch.tensor(test_output_obj['watermark'],
                                     dtype=torch.long,
                                     device=DEVICE)
            ...
            codebleu_res = calc_code_bleu.evaluate_per_example(test_out_str,
                                                               attacked_str,
                                                               lang=LANG)
    ...
    if args.output_txt:
        fo.close()
    if repo_wise:
        pickle.dump([repowise_long_msg, repowise_long_msg_dewm],
