...
from data_processing import (DataInstance, CodeVocab, JsonlWMDatasetProcessor,
                             DynamicWMCollator)
...
def gather_samples(instances: List[DataInstance],
                   objs: List,
                   vocab: CodeVocab,
    ...
    dataloader = DataLoader(dataset,
                            batch_size=1,
                            shuffle=False,
                            collate_fn=DynamicWMCollator(4))
    ...
    with torch.no_grad():
        ...
        for bid, batch in enumerate(prog):
            ...
            dewatermark_obj = {
                ...
                'docstring_tokens':
                obj['docstring_tokens'] if 'docstring_tokens' in obj else None,
...
def main(args):
    ...
    transform_manager = InMemoryJitRuntimeDataManager(transform_computer, all_instances,
                                                      LANG)
    ...
    if DATASET in {'mbjp', 'mbjsp'}:
        detector_test_samples = gather_samples(test_instances, raw_test_objs, vocab,
                                               dataset_processor, transform_manager,
                                               encoder, wm_encoder, selector, DEVICE,
                                               VAR_TRANSFORM_MODE)
    else:

        detector_train_samples = gather_samples(train_instances, raw_train_objs, vocab,
                                                dataset_processor, transform_manager,
                                                encoder, wm_encoder, selector, DEVICE,
                                                VAR_TRANSFORM_MODE)
        detector_valid_samples = gather_samples(valid_instances, raw_valid_objs, vocab,
                                                dataset_processor, transform_manager,
                                                encoder, wm_encoder, selector, DEVICE,
                                                VAR_TRANSFORM_MODE)
        detector_test_samples = gather_samples(test_instances, raw_test_objs, vocab,
