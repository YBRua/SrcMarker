...
def main(args):
    ...
    if MODEL_ARCH == 'gru':
        ...
        rw_encoder = GRUEncoder(vocab_size=len(vocab),
                                hidden_size=FEATURE_DIM,
                                embedding_size=FEATURE_DIM)
        if SHARED_ENCODER:
        ...
        encoder = TransformerEncoderExtractor(vocab_size=len(vocab),
                                              embedding_size=FEATURE_DIM,
                                              hidden_size=FEATURE_DIM)
        rw_encoder = TransformerEncoderExtractor(vocab_size=len(vocab),
                                                 embedding_size=FEATURE_DIM,
                                                 hidden_size=FEATURE_DIM)
        if SHARED_ENCODER:
            extract_encoder = None
    ...
    with torch.no_grad():
        ...
        for bid, batch in enumerate(prog):
            ...
            rw_vs_output = rw_selector.var_selector_forward(rw_code_feature,
                                                            rw_wm_feature)
            rw_vs_ids = torch.argmax(rw_vs_output, dim=1)

            rw_ss_output = rw_selector.transform_selector_forward(rw_code_feature,
                                                                  rw_wm_feature,
                                                                  transform_mask=s_masks)
            ...
            tot_adv_acc += torch.sum(torch.mean((rw_preds == wms).float(), dim=1)).item()
            tot_adv_msg_acc += compute_msg_acc(rw_preds, wms, n_bits=args.n_bits)

            tot_new_acc += torch.sum(torch.mean((rw_preds == rw_wms).float(),
                                                dim=1)).item()
            ...
            for i in range(B):
                ...
                trans_vs_vadv = calc_code_bleu.evaluate_per_example(reference=t_str,
                                                                    hypothesis=rw_str,
                                                                    lang=args.lang)
