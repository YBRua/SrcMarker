    ...
    for block_con in block_cons:
        ...
        for decl in ls_decl:
            ...
            if len(decl.xpath('src:name', namespaces=ns)[0]) != 0:
                ...
                des_index, f, b_ele = judge_ini(
                    var_index, block_con,
                    decl.xpath('src:name', namespaces=ns)[0].text)
...
def trans_temp_var(e):
    ...
    for block_ in block_s:
        ...
        for decl in ls_dec:
            if decl.getparent(
            ).tag == '{http://www.srcML.org/srcML/src}decl_stmt' and decl.getparent(
            ).index(decl) != 0:
                decl.remove(decl.getchildren()[0])
    ...
    for block_con in block_cons:
        ...
        for decl_stml in block_con:
            if decl_stml.tag != '{http://www.srcML.org/srcML/src}decl_stmt':
                break
            for decl in decl_stml:
                if len(decl) == 2 or (len(decl) == 3 and len(decl[2]) > 0
                                      and len(decl[2][0]) > 0 and decl[2][0][0].tag
