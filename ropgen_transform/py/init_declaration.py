                    ...
                    for decl_stmt in decl_stmts:
                        ...
                        for decl in decl_stmt:
                            if len(decl) == 2 or len(
                                    decl
                            ) == 3:  # and decl[2][0][0].tag=='{http://www.srcML.org/srcML/src}literal'):
                                modifier_num = decl[0].xpath("src:modifier",
                                                             namespaces=ns)
                        ...
                        #----------------------------------end---------
                        if len(decl_stmt) != 0 and len(decl_stmt[0]) != 0 and len(
                                decl_stmt[0][0]) != 0 and len(decl_stmt[0][0][0]) == 0:
                            for l in ls:
                                if ''.join(
                                        decl_stmt[0][0].itertext()
                                ).replace(' ', '') == ''.join(l[0][0].itertext()).replace(
                                        ' ', ''
            ...
            if decl_stmt.tag != '{http://www.srcML.org/srcML/src}decl_stmt':
                ...
                for decl_stmt in decl_stmts:
                    ...
                    if len(decl_stmt) != 0 and len(decl_stmt[0]) != 0 and len(
                            decl_stmt[0][0]) != 0 and len(decl_stmt[0][0][0]) == 0:
                        for l in ls:
                            if len(get_decl_type(decl_stmt)) == 0 or len(
                                    get_decl_type(l)) == 0:
                                continue
                            if get_decl_type(decl_stmt)[0].text == get_decl_type(
                                    l)[0].text:
                                num += 1
                                key = 1
                                break
