...
def trans_tree(e):
    ...
    for ternary_elem in ternary_elems:
        ...
        if (
            ternary_elem.getparent().tag == str + "expr"
            and ternary_elem.getparent().getparent().tag == str + "expr_stmt"
        ):
            index = ternary_elem.getparent().index(ternary_elem)
            if (ternary_elem.getparent()[index - 1].text == "=") or (
                ternary_elem.getparent()[index - 1].text == "("
                ...
                elem_1.tail = ";\n}"
                if len(get_expr2(ternary_elem)) == 0:
                    continue
