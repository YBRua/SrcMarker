    ...
    for author in prog:
        ...
        for file in os.listdir(author_src_dir):
            ...
            for i, func in enumerate(funcs):
                tot_funcs += 1
                func_xml_output_path = os.path.join(author_dst_dir,
                                                    f'{file_path_noext}.{i}.xml')
                func_file_output_path = os.path.join(author_dst_dir,
                                                     f'{file_path_noext}.{i}.{LANG}')

                xml_premble = (r'<unit xmlns="http://www.srcML.org/srcML/src" '
                ...
                srcml_interface.srcml_xml_to_program(func_xml_output_path,
                                                     func_file_output_path)
