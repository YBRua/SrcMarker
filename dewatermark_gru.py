...
def parse_args_for_adaptive_removal():
    ...
    parser.add_argument(
        "--dataset_dir", type=str, default="./datasets/detection/github_java_funcs"
    )
    ...
    parser.add_argument(
        "--attack_checkpoint", type=str, default="./ckpts/dewatermark_gru"
    )
    parser.add_argument(
        "--attack_dataset_dir",
        type=str,
...
def main():
    ...
    attack_output_filename = (
        f"{save_dir_name}/{os.path.basename(ATTACK_DATASET_DIR)}_{DATASET}.json"
    )
    ...
    if args.do_train:
        ...
        model = Seq2SeqAttentionGRU(
            vocab_size=len(vocab), hidden_size=FEATURE_DIM, bos_idx=vocab.bos_idx()
        )
        initial_lr = 1e-3
        model.to(device)
        ...
        try:
            ...
            for eid in range(N_EPOCHS):
                train_res = train(
                    eid, train_loader, model, loss_fn, optimizer, vocab, device
                )
                ...
                valid_res, decoded_text = evaluate(
                    eid, valid_loader, model, loss_fn, vocab, device, sample=True
                )
    ...
    if args.do_attack:
        ...
        attack_instances = dataset_processor.load_jsonl(
            ATTACK_DATASET_DIR, split="test"
        )
