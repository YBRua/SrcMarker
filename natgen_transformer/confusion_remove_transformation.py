...
class ConfusionRemoveTransformer(NatGenBaseTransformer):
    ...
    def get_available_transforms(self) -> List[str]:
        return ["incre_decre_removal", "conditional_removal"]

    def transform_incre_decre_removal(self, code: str) -> str:
    ...
    def transform_conditional_removal(self, code: str) -> str:
        if len(self.transformations) == 0:
            return code
        function = self.transformations[1]
        _, modified_code, _ = function(code, self)
        return modified_code

    def transform(self, code: str, transform: str):
        if transform == "incre_decre_removal":
            return self.transform_incre_decre_removal(code)
        elif transform == "conditional_removal":
...
if __name__ == "__main__":
    ...
    for lang in ["c", "cpp", "java", "cs", "python", "php", "go", "ruby"]:
        ...
        confusion_remover = ConfusionRemoveTransformer(parser_path, lang)
        print(lang)
