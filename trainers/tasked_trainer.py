...
class TaskedWMTrainer:
    ...
    def _train_epoch(self, eid: int, dataloader: DataLoader):
        ...
        tot_real_acc = 0.0
        tot_task_loss = 0.0
        tot_approx_task_loss = 0.0

        for bid, batch in enumerate(progress):
            (x, length, padding_mask, instance_ids, wms, wmids, labels) = batch
            ...
            # classification on original code
            task_output = self.classifier(code_feature)
            task_loss = self.task_loss_fn(task_output, labels)
            task_preds = torch.argmax(task_output, dim=1)
            ...
            # compute task loss on approximated feature
            approx_task_outputs = self.classifier(feat_warped)
            approx_task_loss = self.task_loss_fn(approx_task_outputs, labels)
            ...
            tot_task_acc += (task_preds == labels).float().mean().item()
            tot_approx_task_acc += (approx_task_preds == labels).float().mean().item()
            tot_loss += loss.item()
            tot_warp_loss += warper_dist_loss.item()
            tot_decode_loss += decode_loss.item()
            tot_task_loss += task_loss.item()
            tot_approx_task_loss += approx_task_loss.item()

            avg_acc = tot_acc / (bid + 1)
            avg_real_acc = tot_real_acc / (bid + 1)
            avg_task_acc = tot_task_acc / (bid + 1)
            ...
            avg_task_loss = tot_task_loss / (bid + 1)
            avg_approx_task_acc = tot_approx_task_acc / (bid + 1)
    ...
    def _test_epoch(self, eid: int, dataloader: DataLoader):
        ...
        for bid, batch in enumerate(dataloader):
            (x, length, padding_mask, instance_ids, wms, wmids, _) = batch
            B = x.shape[0]
            wm_len = wms.shape[1]
