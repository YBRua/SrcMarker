...
class CodebertWMTrainer:
    ...
    def _train_epoch(self, eid: int, dataloader: DataLoader):
        ...
        for bid, batch in enumerate(progress):
            ...
            tx, tl, tmask = self.transform_manager.load_to_tensor_bert(t_instances)
            tx = tx.to(self.device)
            tmask = tmask.to(self.device)
            tfeatures = self.code_encoder(tx, tl, tmask)

            # computing feature distance loss
            warper_dist_loss = 0.1 * self.warper_loss_fn(feat_warped, tfeatures)
            ...
            loss = decode_loss + warper_dist_loss + t_loss
            loss.backward()
