...
class DiscrimWMTrainer:
    def __init__(
        ...
        self.discrim_loss_fn = nn.BCEWithLogitsLoss()
    ...
    def _train_epoch(self, eid: int, dataloader: DataLoader):
        ...
        for bid, batch in enumerate(progress):
            ...
            # discriminator (positive samples)
            real_logits = self.discriminator(code_feature.detach())
            real_labels = torch.ones_like(real_logits)
            discrim_real_loss = self.discrim_loss_fn(real_logits, real_labels)
            ...
            disc_loss = discrim_real_loss + discrim_fake_loss
            disc_loss.backward()
            ...
            tot_gen_loss += discrim_fake_loss_gen.item()
            ...
            avg_disc_loss = tot_disc_loss / (bid + 1)
